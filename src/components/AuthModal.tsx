...
interface AuthModalProps {
  onClose: () => void;
  onSuccess: () => void; // O que fazer quando logar (ex: fechar e avisar)
}
    ...
    try {
      if (isLogin) {
        ...
        const { error } = await supabase.auth.signUp({
          ...
          password,
          options: {
            // Envia o nome para salvar nos metadados (para nosso trigger criar o profile)
            data: { full_name: fullName, avatar_url: '' } 
          }
        });
        if (error) throw error;
        alert('Conta criada! Se o login não for automático, verifique seu email.');
        onSuccess();
      }
    } catch (err: any) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded-3xl shadow-2xl overflow-hidden relative">
        ...
        {/* Botão Fechar */}
        <button onClick={onClose} className="absolute top-4 right-4 p-2 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-full transition-colors">
        ...
        <div className="p-8">
          ...
          <button 
            ...
            className="w-full py-3 mb-6 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-xl font-bold text-gray-700 dark:text-white hover:bg-gray-50 dark:hover:bg-slate-600 transition-colors flex items-center justify-center gap-2"
          >
          ...
          <div className="relative mb-6 text-center">
             ...
             <span className="relative bg-white dark:bg-slate-800 px-4 text-xs text-gray-400 uppercase font-bold">OU USE EMAIL</span>
          </div>

          <form onSubmit={handleSubmit} className="space-y-4">
            {error && <div className="p-3 bg-red-50 text-red-500 text-xs font-bold rounded-lg">{error}</div>}
