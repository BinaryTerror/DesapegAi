...
import { ShoppingBag, Search, Sun, Moon, LogIn, User, LogOut, PlusCircle, ShieldAlert, CreditCard } from 'lucide-react';
...
interface NavbarProps {
  ...
  userProductCount: number; // NOVO: Quantos produtos o user tem
  onOpenAuth: () => void;
  onOpenPlans: () => void; // NOVO: Abrir modal de planos
}
...
const Navbar: React.FC<NavbarProps> = ({ 
  ...
  // Verifica se é VIP
  const isVip = userProfile?.is_unlimited || (userProfile?.unlimited_until && new Date(userProfile.unlimited_until) > new Date());
      ...
      <div className="max-w-7xl mx-auto px-4 h-20 flex items-center justify-between gap-2 md:gap-4">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          {user && (
             <button 
               onClick={onOpenPlans}
               className="flex items-center gap-1 bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 px-3 py-1.5 rounded-full text-xs font-bold hover:scale-105 transition-transform"
             >
               <CreditCard size={14} />
               <span>{isVip ? 'VIP' : `${userProductCount}/${limit}`}</span>
               <div className="w-4 h-4 bg-orange-500 text-white rounded-full flex items-center justify-center ml-1 text-[10px]">+</div>
             </button>
          )}
                ...
                <>
                  ...
                  <div className="absolute right-0 top-12 w-48 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-gray-100 dark:border-slate-700 overflow-hidden py-2 animate-fade-in z-50">
                    ...
                    {userProfile?.role === 'admin' && (
                      <button onClick={() => { onNavigate('ADMIN'); setIsProfileOpen(false); }} className="w-full text-left px-4 py-3 hover:bg-red-50 dark:hover:bg-red-900/20 text-sm font-bold text-red-600 flex items-center gap-2">
                        <ShieldAlert size={16} /> Admin Panel
                      </button>
                    )}

                    <button onClick={handleLogout} className="w-full text-left px-4 py-3 hover:bg-gray-50 dark:hover:bg-slate-700 text-sm font-medium text-red-500 flex items-center gap-2 border-t border-gray-100 dark:border-slate-700">
                      <LogOut size={16} /> Sair
                    </button>
                  </div>
                </>
