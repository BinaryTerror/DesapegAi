...
import { Filter, ChevronDown, MapPin, X, Grid } from 'lucide-react';
import { Category } from '../types';
import { MOZ_PROVINCES } from '../types'; // Importe a lista que criamos no types
...
const FilterBar: React.FC<FilterBarProps> = React.memo(({ activeCat, activeProv, onSelectCat, onSelectProv }) => {
        ...
        {showCatMenu && (
          <>
            ...
            <div className="absolute top-12 left-0 w-64 bg-white dark:bg-slate-800 p-2 rounded-2xl shadow-xl border border-gray-100 dark:border-slate-700 z-40 animate-fade-in max-h-[300px] overflow-y-auto custom-scrollbar">
               <button onClick={() => { onSelectCat(null); setShowCatMenu(false); }} className="w-full text-left p-2 hover:bg-gray-50 dark:hover:bg-slate-700 rounded-lg text-sm font-bold">Todas as Categorias</button>
        ...
        {showProvMenu && (
          <>
            ...
            <div className="absolute top-12 left-0 w-56 bg-white dark:bg-slate-800 p-2 rounded-2xl shadow-xl border border-gray-100 dark:border-slate-700 z-40 animate-fade-in max-h-[300px] overflow-y-auto custom-scrollbar">
               <button onClick={() => { onSelectProv(null); setShowProvMenu(false); }} className="w-full text-left p-2 hover:bg-gray-50 dark:hover:bg-slate-700 rounded-lg text-sm font-bold">Todo Moçambique</button>
